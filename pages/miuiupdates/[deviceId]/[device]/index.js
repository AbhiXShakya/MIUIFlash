...
import { Breadcrumbs, DeviceUpdatedRoms } from "../../../../components";
...
const device = ({ data }) => {
  ...
  const breadcrumbs = [
    ...
    {
      name: titleCase(data.deviceId),
      href: `/miuiupdates/${data.deviceId}`,
    },
  ];
...
export const getStaticProps = async ({ params }) => {
  ...
  try {
    res = await axios.get(
      `${process.env.REACT_APP_API_URL}/miuiupdates/devices/${deviceId}/${device}?key=THISISSECRET`,
      {
        responseType: "json",
      }
    );
